...
import {
  MIDNIGHT_NAVY, SAPPHIRE, SLATE, CLOUD,
  FROST, WHITE, GRAD_PRIMARY, GRAD_DARK, FONT_SANS, FONT_DISPLAY,
} from "@/lib/brand";
...
export default function HowItWorksPage() {
  return (
    <div style={{ background: BG, color: TEXT, fontFamily: FONT_SANS }}>
      ...
      {/* Header */}
      <section style={{ background: GRAD_DARK, padding: "72px 24px 56px", textAlign: "center" }}>
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(255,255,255,0.5)", marginBottom: 14 }}>
      ...
      <section style={{ background: WHITE, borderTop: `1px solid ${BORDER}`, borderBottom: `1px solid ${BORDER}`, padding: "72px 24px" }}>
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          ...
          {/* Visual timeline */}
          <div style={{ background: FROST, border: `1px solid ${BORDER}`, borderRadius: 14, padding: "28px 24px", marginBottom: 28 }}>
      ...
      <section style={{ maxWidth: 800, margin: "0 auto", padding: "72px 24px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {[
            ...
            <div key={time} style={{ display: "flex", alignItems: "center", gap: 16, background: WHITE, border: `1px solid ${BORDER}`, borderRadius: 10, padding: "16px 20px" }}>
      ...
      <section style={{ maxWidth: 800, margin: "0 auto", padding: "72px 24px" }}>
        ...
        <div style={{ background: WHITE, border: `1px solid ${BORDER}`, borderRadius: 14, padding: "28px 24px", marginBottom: 28 }}>
          <p style={{ fontWeight: 700, fontSize: 15, color: TEXT, marginBottom: 20 }}>Example: 100 inquiries at $100 each = $10,000 in ad spend</p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div style={{ background: "rgba(239,68,68,0.05)", border: "1px solid rgba(239,68,68,0.15)", borderRadius: 10, padding: "16px", textAlign: "center" }}>
              <p style={{ fontSize: 13, color: MUTED, marginBottom: 6 }}>Without ClozeFlow (25%)</p>
              <p style={{ fontSize: 28, fontWeight: 900, color: "#dc2626", marginBottom: 4 }}>25 patients</p>
              <p style={{ fontSize: 12, color: MUTED }}>75 inquiries lost<br />$7,500 in wasted spend</p>
            </div>
            <div style={{ background: "rgba(39,174,96,0.05)", border: "1px solid rgba(39,174,96,0.15)", borderRadius: 10, padding: "16px", textAlign: "center" }}>
              <p style={{ fontSize: 13, color: MUTED, marginBottom: 6 }}>With ClozeFlow (55%)</p>
              <p style={{ fontSize: 28, fontWeight: 900, color: "#27AE60", marginBottom: 4 }}>55 patients</p>
              <p style={{ fontSize: 12, color: MUTED }}>30 extra appointments<br />Same ad spend</p>
            </div>
          </div>
        </div>
      ...
      {/* Section 5: How ClozeFlow Works — Step by Step */}
      <section style={{ background: WHITE, borderTop: `1px solid ${BORDER}`, borderBottom: `1px solid ${BORDER}`, padding: "72px 24px" }}>
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
            ...
            {[
              ...
              <div key={n} style={{ display: "flex", gap: 20 }}>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                  <div style={{ width: 36, height: 36, borderRadius: "50%", background: GRAD_PRIMARY, display: "flex", alignItems: "center", justifyContent: "center", color: WHITE, fontWeight: 900, fontSize: 14, flexShrink: 0 }}>
                    {n}
                  </div>
                  {i < arr.length - 1 && (
      ...
      <section style={{ maxWidth: 800, margin: "0 auto", padding: "72px 24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16 }}>
          {[
            { title: "Tone",          desc: "Conversational, not salesy. Sound like a person who genuinely wants to help.",          icon: "🗣️" },
            { title: "Timing",        desc: "Fast first response. Then persistent follow-up spaced at optimal intervals.",           icon: "⏱️" },
            { title: "Persistence",   desc: "5+ touches over 1–3 weeks. Most appointments are won on the 4th or 5th follow-up.",    icon: "🔁" },
            { title: "Personalization", desc: "Use the patient&apos;s name, mention their specific inquiry, and reference their concern.", icon: "🎯" },
          ].map(({ title, desc, icon }) => (
            <div key={title} style={{ background: WHITE, border: `1px solid ${BORDER}`, borderRadius: 12, padding: "20px" }}>
      ...
      <section style={{ background: GRAD_DARK, padding: "72px 24px", textAlign: "center" }}>
        <div style={{ maxWidth: 560, margin: "0 auto" }}>
          <h2 style={{ fontSize: 32, fontWeight: 900, color: WHITE, marginBottom: 16, letterSpacing: "-0.02em", fontFamily: FONT_DISPLAY }}>
